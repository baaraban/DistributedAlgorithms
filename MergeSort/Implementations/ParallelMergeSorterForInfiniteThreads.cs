...
{
    public class ParallelMergeSorterForInfiniteThreads : IMergeSorter
    {
        ...
        private void internalSort(int[] array, int from, int to)
        {
            ...
            var lTask = Task.Factory.StartNew( () => internalSort(array, from, leftEnd) );
            var rTask = Task.Factory.StartNew( () => internalSort(array, leftEnd + 1, to));
            Task.WaitAll(lTask, rTask);
